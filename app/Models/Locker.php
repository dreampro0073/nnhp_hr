...
class Locker extends Model
{
    ...
    public static function totalShiftData(){
        ...
        $total_shift_upi += DB::table('locker_penalty')->where('date',$p_date)->where('added_by',Auth::id())->where('pay_type',2)->sum("penalty_amount");

        $total_shift_cash = Locker::where('date',$p_date)->where('added_by',Auth::id())->where('deleted',0)->where('pay_type',1)->sum("paid_amount");
        $total_shift_cash += DB::table('locker_penalty')->where('date',$p_date)->where('added_by',Auth::id())->where('pay_type',1)->sum("penalty_amount");

        $last_hour_upi_total = Locker::where('date',$p_date)->where('added_by',Auth::id())->where('deleted',0)->where('pay_type',2)->whereBetween('check_in', [$from_time, $to_time])->sum("paid_amount"); 
        $last_hour_upi_total += DB::table('locker_penalty')->where('date',$p_date)->where('added_by',Auth::id())->where('pay_type',2)->whereBetween('current_time', [$from_time, $to_time])->sum("penalty_amount"); 
        
        $last_hour_cash_total = Locker::where('date',$p_date)->where('added_by',Auth::id())->where('deleted',0)->where('pay_type',1)->whereBetween('check_in', [$from_time, $to_time])->sum("paid_amount");
        $last_hour_cash_total += DB::table('locker_penalty')->where('date',$p_date)->where('added_by',Auth::id())->where('pay_type',1)->whereBetween('current_time', [$from_time, $to_time])->sum("penalty_amount");
